...
struct MeasureGroup {
  ...
  // Closest decoded image to the LiDAR scan end time.
  // Empty (image_frame.empty() == true) when no image is available
  // or lio.ros.image_topic is not set.
  cv::Mat  image_frame;
  double   image_stamp{-1.0};  // seconds — timestamp of the matched frame
};
