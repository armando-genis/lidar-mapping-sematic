...
public:
  ...
  void diagnoseSyncStall() const {
    LOG(WARNING) << YELLOW
      << "  [SyncDiag] lidar_buf=" << lidar_buffer_.size()
      << " imu_buf=" << imu_buffer_.size()
      << " last_ts_lidar=" << std::fixed << std::setprecision(3) << last_timestamp_lidar_
      << " last_ts_imu=" << last_timestamp_imu_
      << (lidar_buffer_.empty() ? "" :
          (std::string(" next_lidar_end=") +
           std::to_string(lidar_buffer_.front().end_time)))
      << RESET;
  }
...
private:
  ...
  // Ring buffer of recent decoded frames: (timestamp_sec, cv::Mat BGR).
  struct DecodedImage { double stamp; cv::Mat frame; };
  std::deque<DecodedImage> image_buffer_;
