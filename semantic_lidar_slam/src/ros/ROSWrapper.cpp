...
void LoadParamFromRos(rclcpp::Node& node)
{
  ...
  node.declare_parameter<bool>("lio.local_map.enable", false);
  node.get_parameter("lio.local_map.enable", g_local_map_enable);

  node.declare_parameter<double>("lio.local_map.radius", 50.0);
  node.get_parameter("lio.local_map.radius", g_local_map_radius);

  node.declare_parameter<int>("lio.local_map.prune_interval", 10);
  node.get_parameter("lio.local_map.prune_interval", g_local_map_prune_interval);

  // ================= loop closure =================
  node.declare_parameter<bool>("lio.loop.enable", false);
  node.get_parameter("lio.loop.enable", g_loop_closure_enable);

  node.declare_parameter<double>("lio.loop.frequency", 1.0);
  node.get_parameter("lio.loop.frequency", g_loop_closure_frequency);

  node.declare_parameter<double>("lio.loop.keyframe_add_dist", 1.0);
  node.get_parameter("lio.loop.keyframe_add_dist", g_loop_keyframe_add_dist);

  node.declare_parameter<double>("lio.loop.keyframe_add_angle", 0.2);
  node.get_parameter("lio.loop.keyframe_add_angle", g_loop_keyframe_add_angle);

  node.declare_parameter<double>("lio.loop.search_radius", 15.0);
  node.get_parameter("lio.loop.search_radius", g_loop_search_radius);

  node.declare_parameter<double>("lio.loop.search_time_diff", 30.0);
  node.get_parameter("lio.loop.search_time_diff", g_loop_search_time_diff);

  node.declare_parameter<int>("lio.loop.search_num", 25);
  node.get_parameter("lio.loop.search_num", g_loop_search_num);

  node.declare_parameter<double>("lio.loop.fitness_score", 0.3);
  node.get_parameter("lio.loop.fitness_score", g_loop_fitness_score);
...
void ROSWrapper::setupIO(){
  ...
  if (!g_image_topic.empty()) {
    auto image_qos = rclcpp::QoS(rclcpp::KeepLast(15))
                     .best_effort()
                     .durability_volatile();
    sub_image_ = this->create_subscription<sensor_msgs::msg::Image>(
        g_image_topic,
        image_qos,
        std::bind(&ROSWrapper::imageHandler, this, std::placeholders::_1),
        sub_opt);
    LOG(INFO) << GREEN << " ---> [ROSWrapper]: Image topic: " << g_image_topic << RESET;
  }
...
void ROSWrapper::imageHandler(const sensor_msgs::msg::Image::SharedPtr msg)
{
  cv::Mat frame;
  ...
  try {
    if (msg->encoding == "mjpeg") {
      ...
      LOG(WARNING) << " ---> [ImageHandler]: Unsupported encoding: " << msg->encoding;
      return;
    }
  } catch (const std::exception& e) {
    LOG(WARNING) << " ---> [ImageHandler]: Decode failed: " << e.what();
    return;
  }
  ...
  // Keep ~1 second of decoded frames at 15fps.
  image_buffer_.push_back({stamp, frame});
  while (image_buffer_.size() > 15)
...
bool ROSWrapper::sync_measure(MeasureGroup& meas){
  ...
  // ── Attach closest decoded frame to LiDAR end time ───────────────────────
  meas.image_frame = cv::Mat();
  meas.image_stamp = -1.0;
  if (!image_buffer_.empty()) {
    double best_dt = std::numeric_limits<double>::max();
    const DecodedImage* best = nullptr;
    for (const auto& img : image_buffer_) {
      double dt = std::abs(img.stamp - meas.lidar.end_time);
      if (dt < best_dt) {
        best_dt = dt;
        best = &img;
      }
