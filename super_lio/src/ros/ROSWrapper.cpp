...
void LoadParamFromRos(rclcpp::Node& node)
{
  ...
  if (g_lidar_type == static_cast<int>(LID_TYPE::RESERVED_LIVOX_REMOVED)) {
    LOG(ERROR) << "lio.sensor.lidar_type=1 (legacy Livox CustomMsg) is unsupported without livox_ros_driver2. "
                    "Publish sensor_msgs/PointCloud2 and set lidar_type to your format (e.g. 4 Velodyne, 2 Hesai).";
  }
