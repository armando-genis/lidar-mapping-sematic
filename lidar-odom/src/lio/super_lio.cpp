...
void SuperLIO::process(){
  if(!data_wrapper_->sync_measure(measures_)){
    const bool in_kf_init  = (state_fn_ == &SuperLIO::stateWaitKFInit);
    const bool in_map_init = (state_fn_ == &SuperLIO::stateWaitMapInit);
    if (in_kf_init || in_map_init) {
      static auto stall_start = std::chrono::steady_clock::now();
      static auto last_warn   = stall_start;
      auto now = std::chrono::steady_clock::now();
      if (std::chrono::duration<double>(now - last_warn).count() >= 2.0) {
        double elapsed = std::chrono::duration<double>(now - stall_start).count();
        const char* stage = in_kf_init ? "KF init" : "map init";
        LOG(WARNING) << YELLOW << " ---> [SuperLIO]: Waiting for " << stage << "... "
                     << std::fixed << std::setprecision(1) << elapsed
                     << " s — sync_measure failing. Check LiDAR/IMU topics are publishing." << RESET;
        data_wrapper_->diagnoseSyncStall();
        last_warn = now;
      }
    }
    return;
