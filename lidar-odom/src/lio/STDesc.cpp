...
// ============================================================================
//  STDescManager — private helpers (identical math to original)
// ============================================================================
      ...
      if (near == voxel_map.end()) {
        ...
        if (!current_octo->is_check_connect_[i]) {
          ...
          if (near_octo->plane_ptr_->is_plane_) {
            Eigen::Vector3d nd = current_octo->plane_ptr_->normal_
                               - near_octo->plane_ptr_->normal_;
            Eigen::Vector3d na = current_octo->plane_ptr_->normal_
                               + near_octo->plane_ptr_->normal_;
            if (nd.norm() < config_setting_.plane_merge_normal_thre_ ||
                na.norm() < config_setting_.plane_merge_normal_thre_) {
    ...
    pcl::PointCloud<pcl::PointXYZINormal>::Ptr &corner_points)
{
  ...
  for (auto &pi : point_list_2d) {
    min_x = std::min(min_x, pi[0]); max_x = std::max(max_x, pi[0]);
    min_y = std::min(min_y, pi[1]); max_y = std::max(max_y, pi[1]);
  }
  ...
  // Use heap-allocated arrays to avoid stack overflow on large scans
  std::vector<double> img_count(x_axis_len * y_axis_len, 0);
  std::vector<double> gradient (x_axis_len * y_axis_len, 0);
  std::vector<double> mean_x_a (x_axis_len * y_axis_len, 0);
  std::vector<double> mean_y_a (x_axis_len * y_axis_len, 0);
  ...
  for (int x = 0; x < x_axis_len; x++) {
    for (int y = 0; y < y_axis_len; y++) {
      ...
      gradient[idx(x,y)] = cnt ? g / cnt : 0;
    }
  }

  // Extract corners per segment
  std::vector<int> max_g_vec, max_gx, max_gy;
  for (int xs = 0; xs < x_segment_num; xs++) {
    for (int ys = 0; ys < y_segment_num; ys++) {
      double max_g = 0; int mx = -1, my = -1;
  ...
  for (size_t i = 0; i < max_g_vec.size(); i++) {
    double px = mean_x_a[idx(max_gx[i],max_gy[i])] / img_count[idx(max_gx[i],max_gy[i])];
    double py = mean_y_a[idx(max_gx[i],max_gy[i])] / img_count[idx(max_gx[i],max_gy[i])];
    Eigen::Vector3d coord = py * x_axis + px * y_axis + proj_center;
    pcl::PointXYZINormal pi;
    pi.x = coord[0]; pi.y = coord[1]; pi.z = coord[2];
    pi.intensity = max_g_vec[i];
  ...
  for (size_t i = 0; i < corner_points->size(); i++) {
    ...
    for (int m = 1; m < near_num - 1; m++) {
      for (int n = m + 1; n < near_num; n++) {
        ...
        // Sort sides a ≤ b ≤ c with matching vertex permutation
        double temp;
        ...
        VOXEL_LOC position((int64_t)(a*1000),(int64_t)(b*1000),(int64_t)(c*1000));
        ...
        auto resolve = [&](const Eigen::Vector3i &la, const Eigen::Vector3i &lb,
                            Eigen::Vector3d &V, Eigen::Vector3d &n, double &att) {
          if      (la[0]==lb[0]) { V<<p1.x,p1.y,p1.z; n<<p1.normal_x,p1.normal_y,p1.normal_z; att=p1.intensity; }
          else if (la[1]==lb[1]) { V<<p2.x,p2.y,p2.z; n<<p2.normal_x,p2.normal_y,p2.normal_z; att=p2.intensity; }
          else                   { V<<p3.x,p3.y,p3.z; n<<p3.normal_x,p3.normal_y,p3.normal_z; att=p3.intensity; }
        };
        resolve(l1, l2, A, normal_1, vertex_attached[0]);
        resolve(l1, l3, B, normal_2, vertex_attached[1]);
        resolve(l2, l3, C, normal_3, vertex_attached[2]);
        ...
        // ── Critical: angle = pairwise vertex-normal dot products × 5 ──────
    ...
    std::vector<STDesc> &stds_vec)
{
  ...
  plane_cloud_vec_.push_back(plane_cloud);   // stored for geometric verify

  // Step 2: build plane adjacency
  build_connection(voxel_map);

  // Step 3: extract corner points
  pcl::PointCloud<pcl::PointXYZINormal>::Ptr corner_points(
      new pcl::PointCloud<pcl::PointXYZINormal>);
  corner_extractor(voxel_map, input_cloud, corner_points);
  corner_cloud_vec_.push_back(corner_points);

  // Step 4: build STDescs from corners
  stds_vec.clear();
  build_stdesc(corner_points, stds_vec);

  // Step 5: free voxel map memory
    ...
    std::vector<STDMatchList> &candidate_matcher_vec)
{
  ...
  for (size_t i = 0; i < stds_vec.size(); i++) {
    ...
    for (auto &vi : voxel_round) {
      ...
      for (size_t j = 0; j < iter->second.size(); j++) {
        ...
        if ((int)(src.frame_id_ - db.frame_id_) <= config_setting_.skip_near_num_)
          continue;
  ...
  for (size_t i = 0; i < useful_match.size(); i++) {
    ...
    for (size_t j = 0; j < useful_match_index[i].size(); j++) {
      ...
      match_index_vec.push_back(db.frame_id_);
    }
  }
    ...
    Eigen::Vector3d &t, Eigen::Matrix3d &rot)
{
  ...
  Eigen::JacobiSVD<Eigen::MatrixXd> svd(src * ref.transpose(),
      Eigen::ComputeThinU | Eigen::ComputeThinV);
  rot = svd.matrixV() * svd.matrixU().transpose();
  if (rot.determinant() < 0) {
    Eigen::Matrix3d K = Eigen::Matrix3d::Identity();
    K(2,2) = -1;
    rot = svd.matrixV() * K * svd.matrixU().transpose();
  }
    ...
    bool skip_geometric_verify)
{
  ...
  for (int i = 0; i < use_size; i++) {
    ...
    int vote = 0;
    for (auto &vp : candidate_matcher.match_list_) {
    ...
    const std::pair<Eigen::Vector3d, Eigen::Matrix3d> &transform)
{
  ...
  for (auto &pi : target_cloud->points) {
    pcl::PointXYZ p; p.x=pi.x; p.y=pi.y; p.z=pi.z;
  ...
  for (auto &sp : source_cloud->points) {
    Eigen::Vector3d pi(sp.x, sp.y, sp.z);
    Eigen::Vector3d ni(sp.normal_x, sp.normal_y, sp.normal_z);
    pi = rot * pi + t;
    ni = rot * ni;

    pcl::PointXYZ q; q.x=pi[0]; q.y=pi[1]; q.z=pi[2];
    if (kd->nearestKSearch(q, 3, ids, sq) <= 0) continue;

    for (int j = 0; j < (int)ids.size(); j++) {
      const auto &np = target_cloud->points[ids[j]];
      Eigen::Vector3d tpi(np.x, np.y, np.z);
      Eigen::Vector3d tni(np.normal_x, np.normal_y, np.normal_z);
      Eigen::Vector3d nd = ni - tni, na = ni + tni;
      double point_to_plane = fabs(tni.transpose() * (pi - tpi));
      if ((nd.norm() < config_setting_.normal_threshold_ ||
           na.norm() < config_setting_.normal_threshold_) &&
          point_to_plane < config_setting_.dis_threshold_) {
        useful++; break;
      }
    }
  }
  return source_cloud->size() > 0 ? useful / source_cloud->size() : 0.0;
}
    ...
    std::pair<Eigen::Vector3d, Eigen::Matrix3d> &transform)
{
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr target_xyz(new pcl::PointCloud<pcl::PointXYZ>);
  for (auto &pi : target_cloud->points) {
    pcl::PointXYZ p; p.x=pi.x; p.y=pi.y; p.z=pi.z; target_xyz->push_back(p);
  }
  kd->setInputCloud(target_xyz);
  ...
  for (auto &sp : source_cloud->points) {
    ...
    pcl::PointXYZ q_pt; q_pt.x=pi_t[0]; q_pt.y=pi_t[1]; q_pt.z=pi_t[2];
    if (kd->nearestKSearch(q_pt, 1, ids, sq) <= 0) continue;

    const auto &np = target_cloud->points[ids[0]];
    Eigen::Vector3d tpi(np.x, np.y, np.z);
    Eigen::Vector3d tni(np.normal_x, np.normal_y, np.normal_z);
    Eigen::Vector3d nd = ni_t - tni, na = ni_t + tni;
    double p2plane = fabs(tni.transpose() * (pi_t - tpi));
    double p2point = (pi_t - tpi).norm();

    if ((nd.norm() < config_setting_.normal_threshold_ ||
         na.norm() < config_setting_.normal_threshold_) &&
        p2plane < config_setting_.dis_threshold_ && p2point < 3.0) {
      problem.AddResidualBlock(
          PlaneSolver::Create(pi, ni, tpi, tni),
          nullptr, para_q, para_t);
    }
  }
  ...
  transform.first  << t_map(0), t_map(1), t_map(2);
}
...
//    std_descs.bin    — hash table (all buckets, all descriptors)
//    std_planes.bin   — plane_cloud_vec_ (one cloud per keyframe)
// ============================================================================
...
bool STDescManager::Save(const std::string &dir) const
{
  ...
  std::cout << "[STD] Database saved to: " << dir
  ...
  // ── 2. Plane clouds ───────────────────────────────────────────────────────
  {
    ...
    for (uint32_t i = 0; i < n_frames; i++) {
      ...
      for (uint32_t j = 0; j < n_pts; j++) {
        ...
        cloud->points[j].x        = buf[0]; cloud->points[j].y        = buf[1];
        cloud->points[j].z        = buf[2]; cloud->points[j].normal_x = buf[3];
        cloud->points[j].normal_y = buf[4]; cloud->points[j].normal_z = buf[5];
      }
      plane_cloud_vec_.push_back(cloud);
    }
    // Restore frame counter so AddSTDescs increments correctly
    current_frame_id_ = n_frames;
  }

  std::cout << "[STD] Database loaded from: " << dir
            << "  buckets=" << data_base_.size()
            << "  frames=" << plane_cloud_vec_.size() << "\n";
  return true;
}
